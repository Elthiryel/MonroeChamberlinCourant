...
If $\alpha^{m}$ is a DPSF then $\alpha^{m}(i)$ can represent the \textit{satisfaction} value of an agent when she is assigned to an alternative ranked $i$'th in her preference order. We assume that for each DPSF $\alpha^{m}(m) = 0$, so an agent is not satisfied at all with her worst alternative. In some cases, we will write $\alpha$ instead of $\alpha^{m}$ to simplify notation.
\\
